...
@PublishedApi
internal tailrec fun <ITEM : Any> FList<ITEM>._size(acc: Int): Int = when (this) {
...
tailrec fun <ITEM : Any, ACC : Any> FList<ITEM>._foldRight(
    base: ACC,
    origin: (ITEM, ACC) -> ACC,
    block: (ACC) -> ACC
): ACC = when (this) {
...
tailrec fun <ITEM : Any, ACC : Any> FList<ITEM>._foldRightWhile(
    base: ACC,
    cond: (ITEM) -> Boolean,
    origin: (ITEM, ACC) -> ACC,
    block: (ACC) -> ACC
...
fun <ITEM : Any, ACC : Any> FList<ITEM>.foldRightWhile(
    base: ACC,
    cond: (ITEM) -> Boolean,
    block: (ITEM, ACC) -> ACC
): ACC = _foldRightWhile(base, cond, block) { it }

fun <ITEM : Any, ACC : Any> FList<ITEM>.foldRight1(base: ACC, block: (ITEM, ACC) -> ACC): ACC = when (this) {
...
fun <ITEM : Any, OTHER : Any> FList<ITEM>.flatMap2(block: (ITEM) -> FList<OTHER>): FList<OTHER> = map(block).flatten()
...
//** append-----------------------------------------------------------------*/
fun <ITEM : Any> FList<ITEM>.append1(list: FList<ITEM> = FList()): FList<ITEM> = when (this) {
...
@PublishedApi
internal tailrec fun <ITEM : Any> FList<ITEM>._dropWhileIndexed(
    index: Int,
    block: (Int, ITEM) -> Boolean
): FList<ITEM> = if (this is Cons && block(index, head)) tail._dropWhileIndexed(index + 1, block) else this
...
//** dropLast----------------------------------------------------------*/
@PublishedApi
internal tailrec fun <ITEM : Any> FList<ITEM>._dropLastWhileIndexed(
    index: Int,
    block: (Int, ITEM) -> Boolean
): FList<ITEM> = when (this) {
...
fun <ITEM : Any> FList<ITEM>.dropLastOne(): FList<ITEM> = when (this) {
...
fun <ITEM : Any> FList<ITEM>.dropLast1(n: Int = 1): FList<ITEM> = when (this) {
...
fun <ITEM : Any> FList<ITEM>.startsWith1(target: FList<ITEM>): Boolean = fold(true to target) { (acc, other), it ->
    when (other) {
...
inline fun <ITEM : Any, OTHER : Any, RESULT : Any> FList<ITEM>.zipWith(
    other: FList<OTHER>,
    noinline block: (ITEM, OTHER) -> RESULT
): FList<RESULT> = fold(FList<RESULT>() to other) { (acc, other), it ->
    when (other) {
